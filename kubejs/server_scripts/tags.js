...
onEvent("tags.blocks", (e) => {
  e.add("allthemodium:other_te_whitelist", [
    "mekanism:teleporter",
    "waystones:waystone",
...
onEvent("tags.items", (e) => {
  ...
  mekanismMetals.forEach((metal) =>
    e.add(`forge:raw_ores/${metal}`, `mekanism:raw_${metal}`)
  );
  immersiveMetals.forEach((metal) =>
    e.add(`forge:raw_ores/${metal}`, `immersiveengineering:raw_${metal}`)
  );

  atmMetals.forEach((metal) =>
    e.add(
      `forge:storage_blocks/raw_${metal}`,
      `allthemodium:raw_${metal}_block`
    )
  );
  e.add(
    "forge:rods/metal",
    atoMetals
      .concat(vanillaMetals, atoAlloys)
      .map((metal) => `alltheores:${metal}_rod`)
  );
  e.add(
    "forge:rods/metal",
    atmMetals.map((metal) => `allthemodium:${metal}_rod`)
  );
