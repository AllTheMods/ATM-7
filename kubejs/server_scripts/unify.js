...
onEvent("recipes", (e) => {
  ...
  function ieUnifyPress(input, type) {
    ...
    if (type === "plate") {
      e.remove({
        id: `immersiveengineering:crafting/plate_${input}_hammering`,
      });
      e.shapeless(output, [
        `2x #forge:ingots/${input}`,
        "#misctags:immersive_engineering_hammer",
  ...
  function occultismUnifyCrusher(input, type) {
    ...
    e.custom({
      ...
      result: Item.of(
        `${craftOverride[input] ?? "alltheores"}:${input}_dust`,
        outputCount
      ),
      crushing_time: time,
  ...
  function blastingUnifyOres(ore) {
    ...
    e.remove({
      ...
      output: `${oreOverride[ore] ?? "alltheores"}:${ore}_ingot`,
      id: `/_dust/`,
    });
    e.blasting(
      `${oreOverride[ore] ?? "alltheores"}:${ore}_ingot`,
      `#forge:dusts/${ore}`
    )
      .xp(0.2)
      .id(`kubejs:blasting/${ore}_ingot_from_dust`);
    e.remove({
    ...
    e.smelting(
      ...
      .xp(0.2)
      .id(`kubejs:smelting/${ore}_ingot_from_dust`);
    ...
    e.blasting(
      ...
      .xp(1.0)
      .id(`kubejs:blasting/${ore}_ingot_from_ore`);
    e.remove({
      type: "minecraft:smelting",
      output: `${oreOverride[ore] ?? "alltheores"}:${ore}_ingot`,
      id: `/_ore/`,
    ...
    e.smelting(
      ...
      .xp(1.0)
      .id(`kubejs:smelting/${ore}_ingot_from_ore`);
  }

  atoMetals.concat(vanillaMetals, atmMetals).forEach((ore) => {
    ["ore", "raw_ore", "raw_block", "ingot", "dust"].forEach((type) =>
      ieUnifyOres(ore, type)
    );
    ["ore", "raw_ore", "raw_block", "ingot"].forEach((type) =>
      createUnifyOres(ore, type)
    );
    ["ore", "raw_ore", "ingot"].forEach((type) =>
      occultismUnifyCrusher(ore, type)
    );
  ...
  vanillaMetals.concat(mekanismMetals).forEach((ore) => {
    ["ore", "raw_ore", "ingot", "dirty_dust"].forEach((type) =>
      mekUnifyOres(ore, type)
    );
  });
  ...
  immersiveMetals.forEach((metal) => {
    e.remove({
      id: `immersiveengineering:crafting/raw_${metal}_to_raw_block_${metal}`,
    });
    e.remove({
      id: `immersiveengineering:crafting/raw_block_${metal}_to_raw_${metal}`,
    });
  });

  immersiveMetals.concat(immersiveAlloys).forEach((metal) => {
    e.remove({
      id: `immersiveengineering:crafting/ingot_${metal}_to_storage_${metal}`,
    });
    e.remove({
      id: `immersiveengineering:crafting/storage_${metal}_to_ingot_${metal}`,
    });
    e.remove({
      id: `immersiveengineering:crafting/ingot_${metal}_to_nugget_${metal}`,
    });
  ...
  e.shapeless(
    ...
    "9x #forge:nuggets/unobtainium"
  );
  e.remove({
    id: "allthemodium:main/unobtainium_nugget_from_unobtainium_ingot",
  });
